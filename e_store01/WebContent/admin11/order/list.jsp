				   ...
				   <c:forEach items="${pageModel.list}" var="o" varStatus="status">
						<tr onmouseover="this.style.backgroundColor = 'white'"
											...
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												...
												<c:if test="${o.state==2}">已发货,未签收</c:if>
												<c:if test="${o.state==3}">结束</c:if>
											</td>
...
$(function(){
	...
	$(".btn").click(function(){
		...
		if("订单详情"==val){
			//发送Ajax请求
			$.post("${pageContext.request.contextPath}/AdminOrderServlet",{"method":"findOrderByOidWithAjax","oid":id},function(dt){
				//alert(dt);
				//var $tb=$(this).next();
