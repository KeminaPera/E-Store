						...
						<tbody>
						  ...
						  <c:forEach items="${cart.cartItems}" var="item">
							<tr class="active">
								...
								<td>
									<a href="javascript:void(0);" onclick="delCart(${item.product.pid})">删除</a>
									<%-- <input type="hidden" name="pid" value="${item.product.pid}"/>
									<a href="CartServlet?method=delCartItem&pid=${item.product.pid}" title="${item.product.pid}" class="delete" id="${item.product.pid}">删除</a>  --%>
								</td>
							</tr>
						  </c:forEach>
						</tbody>
