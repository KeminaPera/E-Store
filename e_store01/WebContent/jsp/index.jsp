...
<html>
	...
	<head>
		...
		<script type="text/javascript">
			$(function(){
				//该部分是通过ajax实现最新商品的加载
				$.get("ProductServlet?method=findNewProducts",function(data){
					$.each(data,function(i,obj){
						/* 遍历回传的数据 */
						/* alert(obj); */
						//定义一个添加模版
						var div = '<div class="col-md-2" style="text-align:center;height:200px;padding:10px 0px;">'+
							'<a href="${pageContext.request.contextPath}/ProductServlet?method=findProductByPid&pid='+obj.pid+'">'+
							'<img src="${pageContext.request.contextPath}/'+obj.pimage+'" width="130" height="130" style="display: inline-block;">'+
							'</a>'+
							'<p><a href="${pageContext.request.contextPath}/ProductServlet?method=findProductByPid&pid='+obj.pid+'" style="color:#666">'+obj.pname+'</a></p>'+
							'<p><font color="#E4393C" style="font-size:16px">&yen;'+obj.shop_price+'</font></p>'+
						'</div>';
						 $("#newProduct").append(div); 
					});
				});
				//该部分是最热商品的加载
				$.get("ProductServlet?method=findHotProducts",function(data){
					$.each(data,function(i,obj){
						/* 遍历回传的数据 */
						/* alert(obj); */
						//定义一个添加模版
